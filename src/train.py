...
def feature_selection():
    ...
    fs_model = xgb.XGBClassifier(use_label_encoder=False, eval_metric='logloss', random_state=42)
...
def tune_train_evaluate_mlflow(model, params, model_name, X_train, y_train, X_test, y_test, sample_weights):
    ...
    grid_search = GridSearchCV(estimator=model, param_grid=params, scoring='f1_weighted', cv=cv, n_jobs=-1, verbose=1)
...
def train():
    ...
    with mlflow.start_run(run_name="Summary_and_Promotion") as summary_run:
        ...
        # Train Models
        lr_run_id, lr_f1, lr_name = tune_train_evaluate_mlflow(lr_model_base, lr_params, "Logistic Regression", X_train_values, y_train_flat, X_test_values, y_test_flat, None)
        results.append({"run_id": lr_run_id, "f1_weighted": lr_f1, "name": lr_name})

        rf_run_id, rf_f1, rf_name = tune_train_evaluate_mlflow(rf_model_base, rf_params, "Random Forest", X_train_values, y_train_flat, X_test_values, y_test_flat, None)
        results.append({"run_id": rf_run_id, "f1_weighted": rf_f1, "name": rf_name})

        xgb_run_id, xgb_f1, xgb_name = tune_train_evaluate_mlflow(xgb_model_base, xgb_params, "XGBoost", X_train_values, y_train_flat, X_test_values, y_test_flat, sample_weights_array)
        results.append({"run_id": xgb_run_id, "f1_weighted": xgb_f1, "name": xgb_name})
        
        # 5. Select Best & Save Production Model
